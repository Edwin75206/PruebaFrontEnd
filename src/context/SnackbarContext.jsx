...
export function SnackbarProvider({ children, autoHideMs = 2500 }) {
  ...
  const [action, setAction] = useState(null); // {label, icon?, onClick?}
  ...
  // Si hay acción, NO autohide para que alcance a pulsarse.
useEffect(() => {
  if (!open) return;
  const t = setTimeout(() => setOpen(false), 3500); // 5 segundos
  return () => clearTimeout(t);
}, [open]);

  const value = useMemo(() => ({ show, showAction, showKey, showKeyAction, close }), [show, showAction, showKey, showKeyAction, close]);

  return (
    <SnackbarContext.Provider value={value}>
      {children}
      {open && <Snackbar message={msg} type={type} action={action} onClose={close} />}
